		...
		<form method="post" id="magicuse_form_${mid}" action="props.action?mid=${mid}&idtype=${idtype }&id=${id }">
			...
			<div class="magic_info">
				...
				<div>
					...
					<table>
						<tr>
						<c:forEach begin="1" end="8" var="value" varStatus="key">
						<td>
							<input type="radio" name="color" value="${value }" id="color${value }" ${value == 1 ? 'checked' : ''} />
							<label for="color${value }" class="magiccolor${value }">■■■</label>
						</td>
						<c:if test="${(key.index - 1) % 4 == 3}"></tr><tr></c:if>
						</c:forEach>
						</tr>
					</table>
				</div>
