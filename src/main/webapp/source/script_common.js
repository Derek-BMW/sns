...
//验证码
function seccode() {
	var img = 'operate.action?ac=seccode&rand='+Math.random();
...
function showFlash(host, flashvar, obj, shareid) {
	var flashAddr = {
		...
		'tudou.com' : 'http://www.tudou.com/FLASHVAR/v.swf',
		'wlkst.com' : 'http://www.wlkst.com/r/cms/www/red/img/flvplayer.swf?FLASHVAR',
		'music' : 'FLASHVAR',
		'flash' : 'FLASHVAR'
	};
	...
	var videoFlash = '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="480" height="450">'
		...
		+ '</object>';
	var musicFlash = '<object id="audioplayer_SHAREID" height="24" width="200" data="image/player.swf" type="application/x-shockwave-flash">'
...
function userapp_open() {
	var x = new Ajax();
	x.get('main.action?ac=common&op=getuserapp', function(s){
...
function userapp_close() {
	var x = new Ajax();
	x.get('main.action?ac=common&op=getuserapp&subop=off', function(s){
		$('my_userapp').innerHTML = s;
...
function showreward() {
	...
	var x = new Ajax();
	x.get('operate.action?ac=ajax&op=getreward', function(s){
		if(s) {
			msgwin(s, 2000);
		}
	});
...
/* 
检查输入字符串是否只由英文字母和数字组合，并且以字母开头
输入：字符串 
返回：如果通过验证返回true,否则返回false 
*/ 
function isNumberOrLetter( s ){
	var regu = "^[a-z]+([a-zA-Z0-9])+$"; 
