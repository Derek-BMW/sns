...
function insertAttach(id) {
	...
	if(is_ie || userAgent.indexOf('firefox') >= 1) {
		...
		if(is_ie2){
			inhtml += '<td><div style="position:relative;width:60px;height:80px;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\''+picPath+'\', sizingMethod=\'scale\');"><input type="hidden" id="picpath_'+id+'" value="'+picPath+'"/></div>&nbsp;</td>';
		}else{
			inhtml += '<td><img id="img_'+id+'" src="' + picPath +'" width="60" height="80"/>&nbsp;</td>';
		}
	}
	if(typeof no_insert=='undefined' || insertType==0) {
...
function getPath(obj){
	if (obj) {
		if (is_ie2) {
			...
			return obj.value;
		}else if(window.navigator.userAgent.indexOf("Firefox")>=7) {
				if (obj.files) {
					// Firefox下取得的是图片的数据
...
function insertAttachimgTag(id) {
	if(insertType == 0) {
		...
		var picPath =$('picpath_' + id).value;
		var imgCache = new Image();
		imgCache.src = picPath;
		edit_insert('<img id="_sns_localimg_' + id + '" style="filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\''+picPath+'\')" src=\'image/none.gif\'/>');
	}else if($('img_' + id)){
		var picPath =$('img_' + id).src;
		edit_insert('<img id="_sns_localimg_' + id + '" src="'+picPath+'"/>');
	}else{
		edit_insert('[local]'+id+'[/local]');
	}
}
...
function upload() {
	...
	if(nowUid>0) {
		...
		if(uploadStat==1) {
			...
			upobj.innerHTML = '上传成功,'+uploadMessage;
		}else {
			upobj.style.color = "#f00";
