...
function wall_add(cid, result) {
	if(result) {
		...
		var x = new Ajax();
		x.get('operate.action?ac=ajax&op=comment', function(s){
			newli.innerHTML = s;
		});
...
function share_add(sid, result) {
	if(result) {
		...
		var newli = document.createElement("div");
		var x = new Ajax();
		x.get('operate.action?ac=ajax&op=share', function(s){
			newli.innerHTML = s;
		});
		obj.insertBefore(newli, obj.firstChild);
...
function comment_edit(id, result) {
	if(result) {
		...
		var x = new Ajax();
		x.get('operate.action?ac=ajax&op=comment&cid='+ cid, function(s){
			obj.innerHTML = s;
		});
	}
}
...
function friend_changegroup(id, result) {
	if(result) {
		...
		var x = new Ajax();
		x.get('operate.action?ac=ajax&op=getfriendgroup&uid='+uid, function(s){
			obj.innerHTML = s;
		});
	}
}
...
function friend_changegroupname(id, result) {
	if(result) {
		...
		var x = new Ajax();
		x.get('operate.action?ac=ajax&op=getfriendname&group='+group, function(s){
			obj.innerHTML = s;
		});
	}
}
...
function post_add(pid, result) {
	if(result) {
		...
		var newli = document.createElement("div");
		var x = new Ajax();
		x.get('operate.action?ac=ajax&op=post', function(s){
			newli.innerHTML = s;
		});
		obj.appendChild(newli);
		if($('message')) {
...
function post_edit(id, result) {
	if(result) {
		...
		var x = new Ajax();
		x.get('operate.action?ac=ajax&op=post&pid='+ pid, function(s){
			obj.innerHTML = s;
		});
	}
}
...
function myfriend_post(id, result) {
	if(result) {
		var ids = explode('_', id);
		var uid = ids[1];
		$('friend_'+uid).innerHTML = '<p>你们现在是好友了，接下来，您还可以：<a href="zone.action?uid='+uid+'#comment" target="_blank">给TA留言</a> ，或者 <a href="main.action?ac=poke&op=send&uid='+uid+'" id="a_poke_'+uid+'" onclick="ajaxmenu(event, this.id, 1)">打个招呼</a></p>';
	}
}
//删除好友请求
...
function picView(albumid) {
	if(albumid == 'none') {
		...
		ajaxget('operate.action?ac=ajax&op=album&id='+albumid+'&ajaxdiv=albumpic_body', 'albumpic_body');
	}
}
//删除重发邮件
...
function docomment_get(id, result) {
	if(result) {
		...
		ajaxget('main.action?ac=doing&op=getcomment&doid='+doid, showid);
...
function docomment_form(doid, id) {
	...
	ajaxget('main.action?ac=doing&op=docomment&doid='+doid+'&id='+id, showid);
	if($(divid)) {
		$(divid).style.display = '';
	}
}
...
function feedcomment_get(feedid, result) {
	...
	ajaxget('main.action?ac=feed&op=getcomment&feedid='+feedid, showid);
	if($(opid) != null) {
		$(opid).innerHTML = '收起';
		$(opid).onclick = function() {
...
function show_click(id) {
	...
	var id = ids[2];
	var clickid = ids[3];
	ajaxget('main.action?ac=click&op=show&clickid='+clickid+'&idtype='+idtype+'&id='+id, 'click_div');
	//提示获得积分
	showreward();
}
...
function getgroup(gid) {
	if(gid) {
		var x = new Ajax();
		x.get('main.action?ac=privacy&op=getgroup&gid='+gid, function(s){
			s = s + ' ';
