...
<div class="mainarea">
	<div class="maininner">
		<div class="tabs_header">
			<ul class="tabs">
				<li${actives.view}><a href="backstage.action?ac=cron"><span>浏览全部</span></a></li>
				<li class="null"><a href="backstage.action?ac=cron&op=add">添加新计划任务</a></li>
			</ul>
		</div>
		<form method="post" action="backstage.action?ac=cron&cronid=${cronid}">
						...
						<table cellspacing="0" cellpadding="0" class="formtable">
							...
							<c:forEach items="${list}" var="cron">
								<tr>
									...
									<td>
										<a href="backstage.action?ac=cron&op=edit&cronid=${cron.cronid }">编辑</a>
										<c:if test="${cron.available > 0}">&nbsp;|&nbsp;<a href="backstage.action?ac=cron&op=run&cronid=${cron.cronid }">执行</a></c:if>
										<c:if test="${cron.type != 'system'}">&nbsp;|&nbsp;<a href="backstage.action?ac=cron&op=delete&cronid=${cron.cronid }">删除</a></c:if>
									</td>
								</tr>
							</c:forEach>
						</table>
