...
function renewContent() {
	if(window.confirm('您确定要恢复上次保存?')) {
		var oArea = parent.$('sns-ttHtmlEditor');
	...
	if(!para){
		...
		if(type == 'insertHTML') {
			if(window.Event){
				...
				obj.pasteHTML(para);
				if(/<img.*id=\"(face_id_[0-9]+)\".*src=\"([^\"]*)\"[^\/]*\/>/gi.test(para)){
					var faceImg=f.document.getElementById(RegExp.$1);
					if(faceImg){
						faceImg.src=RegExp.$2;
						faceImg.removeAttribute("id");
					}
				}
				obj.collapse(false);
				obj.select();
...
function faceBox(e) {
	...
	for(i=1; i<31; i++) {
		...
		faceli.innerHTML = '<img src="image/face/'+i+'.gif" id="'+i+'" onclick="insertImg(this);" style="cursor:pointer;" />';
...
function insertImg(img) {
	var id='';
	if(gIEVer>0){
		id='id="face_id_'+img.id+'"';
	}
	var src=img.src;
	src=src.substring(src.indexOf('image/face'));
	format("insertHTML", '<img '+id+' src="' + src + '"/>');
}
